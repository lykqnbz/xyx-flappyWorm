...
export class Director {
  ...
  run() {
    ...
    this.check();
    const score = this.dataStore.get('score');
    const maxScore = this.dataStore.get('maxScore');
    if (!this.isGameOver) {
      //绘制背景
      ...
      //创建铅笔
      if (pencils[0].x <= (DataStore.getInstance().canvas.height - pencils[0].width) / 2 && pencils.length === 2) {
        this.createPencil();
      }
      //绘制铅笔
      ...
      this.dataStore.get('maxScore').draw();
